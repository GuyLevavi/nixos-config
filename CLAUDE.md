...
**Desktop geometry runs on two constants.** **4 = alignment** — `gaps_out = 4` / `gaps_in = 2` (Hyprland's `gaps_in` is a half-gap) plus `margin_edge` / `padding = 4` on the bar, so island outer edges and window frames land on the same logical x. **8 = separation** — `widget_spacing` (island-to-island), `margin_opposite_edge = 4` (making bar-to-window 8), `decoration.rounding`, island `radius`. Don't collapse them into one number; at a uniform 4 the bar read as a single slab. `margin_ends` and `padding` **stack**, so `margin_ends` must be 0. Radii follow the concentric rule (`outer = inner + gap`): pills `capsule_radius = 2` (8 − 6 padding), screen corners `12` (8 + 4).

**Noctalia cannot match Hyprland's window shadows.** `bar.default.shadow = true` grows the layer surface but draws *nothing* under the islands — it only hazes the top 6px of the windows beneath, since the bar is `layer = "top"`. `contact_shadow` is a no-op. Both stay off. `margin_opposite_edge` is what compensates: it adds to the exclusive zone without moving the bar surface (reserved 34 → 38, surface unchanged), because the window shadow ramps ~5px and swallowed a 4px gap whole.
